...
export function registerStoreCommands(program: Command): void {
  ...
  // ── REGISTER store (vendor self-service) ──────────────────────────────
  store
    .command("register")
    .description("Register your vendor store with CLISHOP")
    .option("--name <name>", "Store name")
    .option("--api-url <url>", "Your vendor API URL (e.g. https://your-store.vercel.app/v1)")
    .option("--description <desc>", "Store description")
    .option("--country <country>", "Country code (e.g. US, BE, NL)")
    .option("--currency <currency>", "Currency (default: EUR)")
    ...
    .action(async (opts) => {
      try {
        ...
        let contactEmail = opts.email;

        // Interactive mode if options not provided
        if (!name || !apiUrl) {
          console.log(chalk.bold.cyan("\n🏪 Register Your Store with CLISHOP\n"));
          console.log(chalk.dim("  Your store must have a running Vendor API that follows the CLISHOP spec."));
          console.log(chalk.dim("  See: https://github.com/DavooxBv2/CLISHOP-VENDOR\n"));
          ...
          const answers = await inquirer.prompt([
            {
              ...
              message: "Store name:",
              default: name,
              validate: (v: string) => v.trim().length >= 2 || "Name must be at least 2 characters",
            },
            {
              type: "input",
              name: "apiUrl",
              message: "Vendor API URL:",
              default: apiUrl,
              validate: (v: string) => {
                if (!v.startsWith("http")) return "Must be a valid URL (https://...)";
                return true;
              },
            },
            {
            ...
            },
            {
              type: "input",
              name: "country",
              message: "Country code (optional, e.g. US, BE):",
              default: country || "",
            },
            {
              type: "input",
          ...
          contactEmail = answers.contactEmail || undefined;
        }

        const spinner = ora("Registering store...").start();
        spinner.text = "Validating vendor API...";
        ...
        spinner.succeed(chalk.green("Store registered successfully!"));

        const { store: s, sync } = res.data;
        ...
        console.log(chalk.dim(`  API URL:  ${s.apiUrl}`));
        console.log(chalk.dim(`  Currency: ${s.currency}`));
        console.log();

        if (sync.created != null) {
          console.log(chalk.green(`  ✓ Synced ${sync.created} products from your vendor API`));
          if (sync.errors?.length > 0) {
            console.log(chalk.yellow(`  ⚠ ${sync.errors.length} sync errors (see --json for details)`));
          }
        } else {
          console.log(chalk.yellow(`  ⚠ ${sync.message}`));
        }

        console.log();
        console.log(chalk.dim("  Your products are now searchable on CLISHOP!"));
        console.log(chalk.dim(`  View store: clishop store info ${s.slug}`));
        console.log(chalk.dim(`  Browse catalog: clishop store catalog ${s.slug}`));
        console.log(chalk.dim(`  Sync products: clishop store sync ${s.slug}`));
        console.log();
      } catch (error) {
        handleApiError(error);
      }
  ...
  store
    .command("sync <store>")
    .description("Sync products from a store's vendor API")
    .option("--json", "Output raw JSON")
    .action(async (storeId: string, opts) => {
      try {
        const spinner = ora(`Syncing products for "${storeId}"...`).start();
        const api = getApiClient();
        const res = await api.post(`/stores/${encodeURIComponent(storeId)}/sync`);
        ...
        const { store: s, created, updated, errors } = res.data;
        console.log();
