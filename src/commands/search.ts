...
export function registerSearchCommands(program: Command): void {
    ...
    .action(async (query: string, opts) => {
      try {
        ...
        if (allProducts.length === 0) {
          ...
          // ── Cache results for `clishop buy <number>` ────────────
          const cacheItems: SearchResultCache[] = allProducts
            .filter((p) => p.id)
            .map((p) => ({
              id: p.id!,
              name: p.name,
          ...
          // Hint about buying by number
          if (cacheItems.length > 0) {
            console.log(
              chalk.dim("  💡 ") +
                chalk.white("Buy a result by number: ") +
                chalk.cyan("clishop buy 1") +
                chalk.dim(" (buys the first result)")
            );
            console.log();
          }
        }
        ...
        if (opts.interactive && allProducts.length > 0) {
          ...
          console.log(
            chalk.dim("  ℹ️  ") +
            chalk.white("Get more details by number: ") +
            chalk.cyan(`clishop info 1`) +
            chalk.dim(" or multiple: ") +
            chalk.cyan(`clishop info 1 2 3`)
          );
          console.log(
            chalk.dim("         Or use ") +
  ...
  program
    .command("info <idsOrNumbers...>")
    .description("Request detailed information about search result products (use IDs or result numbers like 1 2 3)")
    .option("--json", "Output raw JSON")
    .action(async (idsOrNumbers: string[], opts) => {
      try {
        if (idsOrNumbers.length === 0) {
          console.error(chalk.red("\n✗ Please provide one or more product IDs or search result numbers."));
          console.log(chalk.dim("  Usage: clishop info <number|id> [number|id...]"));
          console.log(chalk.dim("  Example: clishop info 1 2 3"));
          console.log(chalk.dim("  Example: clishop info xprd_abc123 xprd_def456\n"));
          process.exit(1);
        }

        if (idsOrNumbers.length > 20) {
          console.error(chalk.red("\n✗ Maximum 20 products per request."));
          process.exit(1);
        }

        // Resolve numbers to product IDs from cached search results
        const cached = getConfig().get("lastSearchResults") || [];
        const ids: string[] = [];
        for (const input of idsOrNumbers) {
          const asNumber = parseInt(input, 10);
          if (!isNaN(asNumber) && asNumber > 0 && String(asNumber) === input.trim()) {
